...
show_usage_and_exit() {
	echo
	echo -e "\t$(tbold)$SCRIPT_NAME$(treset) - Tesla SSH Hsteria server install script"
	echo
	echo -e "Usage:"
	echo
	echo -e "$(tbold)Install Tesla UDP Hysteria$(treset)"
	...
	echo -e "\t-l, --local <file>\tInstall specified TeslaSSH binary instead of download it."
	echo -e "\t--version <version>\tInstall specified version instead of the latest."
	echo
	echo -e "$(tbold)Remove Tesla udp Hysetria$(treset)"
	echo -e "\t$0 --remove"
	echo
...
tpl_hysteria_server_service_base() {
  ...
  cat << EOF
[Unit]
Description=TeslaSSH UDP Service
